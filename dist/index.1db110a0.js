...
RGraph.Bar = function(conf) {
    ...
    for(var i1 in this.properties)if (typeof i1 === "string") this.properties_lowercase_map[i1.toLowerCase()] = i1;
    // Check for support
    if (!this.canvas) {
            ...
            if (typeof this.data[i1] == "number") {
                ...
                // Regular bar
                    ...
                    // Glass variant
                        ...
                        this.context.roundRect(x + hmargin + 3, y + (this.data[i1] > 0 ? 3 : 0), barWidth / 2 - 2, height - 2, this.data[i1] > 0 ? [
                            RGraph.isNumber(properties.cornersRoundRadius) ? properties.cornersRoundRadius - 3 : 5,
                            0,
                            0,
                            0
                        ] : [
    ...
    this.getShape = function(e) {
        ...
        for(var i1 = 0, len1 = coords1.length; i1 < len1; i1 += 1){
            if (this.coords[i1].length == 0) continue;
            if (RGraph.tooltipsHotspotIgnore(this, i1)) continue;
            ...
            if (this.context.isPointInPath(mouseX, mouseY) && (this.properties.clip ? RGraph.clipTo.test(this, mouseX, mouseY) : true)) {
        ...
        if (labels && grouping === "grouped") for(var i1 = 0, len1 = data.length, sequentialIndex = 0; i1 < len1; i1 += 1){
            ...
            if (typeof data[i1] === "number" && data[i1] >= 0) {
                ...
                RGraph.text({
                    ...
                    y: coords2[i1][0][1] - offsety - 3,
                    text: specific ? specific[sequentialIndex] || "" : RGraph.numberFormat({
                        object: this,
                        number: Number(typeof data[i1] === "object" ? data[i1][0] : data[i1]).toFixed(decimals),
                        value: Number(typeof data[i1] === "object" ? data[i1][0] : data[i1]).toFixed(decimals),
                        unitspre: unitsPre,
                        unitspost: unitsPost,
                        point: point,
                        thousand: thousand,
            ...
            // Alignment for regular, negative bars
                ...
                RGraph.text({
                    ...
                    italic: textConf.italic,
                    x: coords2[i1][0][0] + coords2[i1][0][2] / 2 + offsetx,
                    y: coords2[i1][0][1] + coords2[i1][0][3] + offsety + 5,
                    text: specific ? specific[sequentialIndex] || "" : RGraph.numberFormat({
                        object: this,
                        number: Number(typeof data[i1] === "object" ? data[i1][0] : data[i1]).toFixed(decimals),
                        value: Number(typeof data[i1] === "object" ? data[i1][0] : data[i1]).toFixed(decimals),
            ...
            // Alignment for grouped bars
                ...
                RGraph.text({
                    ...
                    y: properties.xaxisPosition === "top" ? coords2[i1][j][1] + coords2[i1][j][3] + 5 : coords2[i1][j][1] + (data[i1][j] < 0 ? coords2[i1][j][3] + offsety + 5 : -offsety),
                    text: specific ? specific[sequentialIndex] || "" : RGraph.numberFormat({
                        object: this,
                        number: Number(data[i1][j]).toFixed(decimals),
                        value: Number(data[i1][j]).toFixed(decimals),
                        unitspre: unitsPre,
                        unitspost: unitsPost,
                        point: point,
                        thousand: thousand,
            ...
            for(var i1 = 0, len1 = data.length, sequentialIndex = 0; i1 < len1; i1 += 1)if (typeof data[i1] === "object") {
                ...
                RGraph.text({
                    ...
                    italic: textConf.italic,
                    x: coords2[i1][0][0] + coords2[i1][0][2] / 2 + offsetx,
                    y: coords2[i1][0][1] + (data[i1][0] < 0 ? coords2[i1][0][3] : 0) + (properties.xaxisPosition === "top" ? coords2[i1].reduce((acc, curr)=>acc += curr[3], 0) + 3 + offsety : -3 - offsety),
                    text: specific ? specific[sequentialIndex] || "" : RGraph.numberFormat({
                        object: this,
                        number: Number(RGraph.arraySum(data[i1])).toFixed(decimals),
                        value: Number(RGraph.arraySum(data[i1])).toFixed(decimals),
                        unitspre: unitsPre,
                        unitspost: unitsPost,
                        point: point,
                        thousand: thousand,
            ...
            //
                ...
                RGraph.text({
                    ...
                    italic: textConf.italic,
                    x: coords2[i1][0][0] + coords2[i1][0][2] / 2 + offsetx,
                    y: coords2[i1][0][1] + (data[i1][0] < 0 ? coords2[i1][0][3] : 0) + (properties.xaxisPosition === "top" ? coords2[i1][0][3] + offsety + 3 : 0 - offsety - 3),
                    text: specific ? specific[sequentialIndex] || "" : RGraph.numberFormat({
                        object: this,
                        number: Number(data[i1]).toFixed(decimals),
                        value: Number(data[i1]).toFixed(decimals),
                        unitspre: unitsPre,
                        unitspost: unitsPost,
                        point: point,
                        thousand: thousand,
    ...
    this.wave = function() {
        ...
        if (this.data.length === 1) return this.grow(arguments[0], arguments[1]);
        ...
        function iterator() {
            ...
            ++frame;
            for(let i1 = 0, len1 = obj.data.length; i1 < len1; i1 += 1)if (frame > opt.startFrames[i1]) {
                ...
                } else if (!RGraph.isNull(obj.data[i1])) for(let j = 0, len2 = obj.data[i1].length; j < len2; j += 1){
    ...
    this.roundedCornersRect = function(x, y, width, height) {
        ...
        // LHS radius
        if (RGraph.isNumber(properties.cornersRoundLeftRadius)) radiusLeft = properties.cornersRoundLeftRadius;
        else radiusLeft = Math.min(width / 2, height / 2, properties.cornersRoundRadius);
        // RHS radius
        if (RGraph.isNumber(properties.cornersRoundRightRadius)) radiusRight = properties.cornersRoundRightRadius;
        else radiusRight = Math.min(width / 2, height / 2, properties.cornersRoundRadius);
    ...
    this.getKeyValue = function(index) {
        if (RGraph.isArray(this.properties.keyFormattedValueSpecific) && RGraph.isNumber(this.properties.keyFormattedValueSpecific[index])) return this.properties.keyFormattedValueSpecific[index];
        else {
            var total = 0;
            for(let i1 = 0; i1 < this.data.length; ++i1)if (RGraph.isArray(this.data[i1])) total += this.data[i1][index];
            return total;
        }
    };
    //
    ...
    this.getKeyNumDatapoints = function() {
        var num = 0;
        for(let i1 = 0; i1 < this.data.length; ++i1)if (RGraph.isArray(this.data[i1])) num = Math.max(num, this.data[i1].length);
        return num;
    };
    //
    ...
    this.clipToScaleWorker = function(clip) {
        ...
        var width = this.canvas.width, y1 = this.getYCoord(from), y2 = this.getYCoord(to), height = Math.abs(y2 - y1), x = 0, y = Math.min(y1, y2);
...
RGraph.CombinedChart.prototype.draw = function() {
    if (RGraph.isArray(this.objects)) {
        for(var i1 = 0; i1 < this.objects.length; ++i1)if (this.objects[i1].properties["combinedEffect"]) {
            // The options must be given as a string because of the
            // RGraph configuration system
            var options = this.objects[i1].properties["combinedEffectOptions"] ? eval("(" + this.objects[i1].properties["combinedEffectOptions"] + ")") : null, callback = this.objects[i1].properties["combinedEffectCallback"], func = this.objects[i1].properties["combinedEffect"];
            this.objects[i1][func](options, callback);
        } else this.objects[i1].draw();
    }
    return this;
};
//
...
//# sourceMappingURL=index.1db110a0.js.map
